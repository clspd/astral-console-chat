...
export default function CommandSuggestion({ input, onAutocomplete }: CommandSuggestionProps) {
  ...
  useInput(
    ...
    { isActive: isCommandMode && matches.length > 0 },
  );
    ...
    <Box flexDirection="column" paddingX={1}>
      {matches.map((cmd, i) => {
        ...
        return (
          <Box key={cmd.name}>
            {isSelected ? (
              <Text color="cyan" inverse>{'  /' + cmd.name}</Text>
            ) : (
              <Text>{'  /' + cmd.name}</Text>
            )}
            {cmd.description && (
